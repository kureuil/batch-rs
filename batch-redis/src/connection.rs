...
pub struct Connection {
	inner: redis::async::SharedConnection,
}
...
impl Connection {
	...
	{
		OpenFuture {
			state: OpenFutureState::Initial { params: Some(params) }
		}
	}
}
...
impl batch::Client for Connection {
	...
	type ToConsumerFuture = Box<futures::Future<Item = Self::Consumer, Error = failure::Error> + Send>;

	fn to_consumer(&mut self, queues: impl IntoIterator<Item = impl AsRef<str>>) -> Self::ToConsumerFuture {
		unimplemented!();
	}
}
...
enum OpenFutureState<P> {
	...
	Connecting {
		fut: Box<futures::Future<Item = redis::async::Connection, Error = redis::RedisError> + Send>,
	},
	Sharing {
		fut: Box<futures::Future<Item = redis::async::SharedConnection, Error = redis::RedisError> + Send>,
	},
}
			...
			OpenFutureState::Connecting { ref mut fut } => match fut.poll() {
				Ok(futures::Async::Ready(conn)) => {
					self.state = OpenFutureState::Sharing {
						fut: redis::async::SharedConnection::new(conn),
					};
					self.poll()
				},
